...
- docker build -t minecraft-server-quilt .
- docker run -d --restart=unless-stopped -v /data/:/data/ -v /server/mods:/server/mods -p 25565:25565 -p 25575:25575 minecraft-server-quilt
...
- 模组文件默认读取`/server/mods/`路径，可选择 -v /server/mods:/server/mods
  - `qfapi.jar`基础模组会自动添加到mods中，配置挂载也不会影响。
...
- RCON远程访问密码，每次构建镜像都会生成一个随机的 30 位大小写字母和数字组合的密码，基于`tr -dc 'A-Za-z0-9'`
  - 可选择
    1. 通过自有配置覆盖来修改密码
    2. 通过`docker exec -it minecraft-server-quilt cat /server/server.properties`查询密码
